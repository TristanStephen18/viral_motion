...
export const CurveLineTrendDatasetTable: React.FC<{
  handleRemoveDataset: (index: number) => void;
  curveLineData: CurveLineTrendDataset[];
  isRendering: boolean;
}> = ({ handleRemoveDataset, curveLineData, isRendering }) => {
  return (
    <Box>
      <Box
        sx={{
      ...
      {curveLineData.map((dataset, i) => (
        ...
        >
          ...
          <Box sx={{ flex: 1 }}>{dataset.dataType}</Box>
